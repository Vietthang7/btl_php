...
<style>
/* Đảm bảo container có width chuẩn */
.container {
    max-width: 1200px;
    margin: 0 auto;
    padding: 0 15px;
...
document.addEventListener('DOMContentLoaded', function() {
    // Thêm class container vào các thẻ cha nếu cần
    if (!document.querySelector('.container')) {
        ...
        if (!mainContent) {
            ...
            // Lấy tất cả phần tử con trực tiếp của body
            const bodyChildren = Array.from(document.body.children);
            
            // Tìm phần tử page-content
            const pageContent = document.querySelector('.page-content');
            if (pageContent) {
                // Di chuyển nội dung vào container
                const pageContentChildren = Array.from(pageContent.children);
                for (const child of pageContentChildren) {
                    if (!child.classList.contains('container') && child.tagName !== 'SCRIPT') {
